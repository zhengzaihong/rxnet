...
class _EnhancedExampleState extends State<EnhancedExample> {
  ...
  Widget _buildSectionWithStop(
      String title, VoidCallback onPressed, VoidCallback onStop) {
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  void _adapterExample1() async {
    try {
      ...
      setState(() {
        result = "✅ 默认适配器示例\n"
            "适配器：DioAdapter（默认）\n"
            "状态：${response.isSuccess ? '成功' : '失败'}\n"
            "数据：${response.value?.toString().substring(0, 100)}...";
      });
    } catch (e) {
      setState(() {
        result = "❌ 错误：$e";
  ...
  void _adapterExample3() async {
    try {
      ...
      setState(() {
        result = "✅ MockAdapter 示例\n"
            "适配器：MockAdapter（测试用）\n"
            "状态：${response.isSuccess ? '成功' : '失败'}\n"
            "数据：${jsonEncode(response.value)}\n"
            "说明：这是模拟数据，无需真实网络请求";
      });
    } catch (e) {
      setState(() {
...
class MultiAdapterExample {
  ...
  static Future<void> fetchMainData() async {
    final result = await mainApi.getRequest().setPath("/api/data").request();
  ...
  static Future<void> sendAnalytics(Map<String, dynamic> data) async {
    final result = await analyticsApi
        .postRequest()
        .setPath("/api/events")
        .setBodyParams(data)
        .asJson()
  ...
  static Future<void> runTest() async {
    final result = await testApi.getRequest().setPath("/api/test").request();
...
class FormApiExample {
  ...
  static Future<void> login(String username, String password) async {
    final result = await RxNet.post()
        ...
        .setBodyParams({"username": username, "password": password})
